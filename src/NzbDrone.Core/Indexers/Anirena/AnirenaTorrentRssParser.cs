...
{
    public class AnirenaTorrentRssParser : RssParser
    {
        ...
        public AnirenaTorrentRssParser()
        {
        ...
        protected override string GetGuid(XElement item)
        {
            var result = item.TryGetValue("link");

            return string.IsNullOrEmpty(result) ? base.GetGuid(item) : result;
        }
        ...
        private string ParseSizeDescription(string description) => description?.Split("-")?[0]?.Split("downloads,")?[1]?.Trim() ?? "";
        ...
        protected override long GetSize(XElement item)
        {
            ...
            if (size == 0 && SizeElementName.IsNotNullOrWhiteSpace())
            {
                ...
                if (itemsize != null)
                {
                    size = ParseSize(ParseSizeDescription(itemsize.Value), true);
                }
            }
